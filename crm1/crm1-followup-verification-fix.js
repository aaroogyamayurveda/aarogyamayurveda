...
(async()=>{
  ...
  async function renderVerification(){
    ...
    try{
      const {data,error}=await db.from('orders').select('id,order_no,verification_status,total_amount,order_date,remarks,customers(customer_name,mobile)').order('order_date',{ascending:false}).limit(500);
