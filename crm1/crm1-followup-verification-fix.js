...
(async()=>{
  ...
  function followupModal(orderId){
    ...
    modal.querySelector('#crm1FollowupCancel').onclick=close;
    modal.onclick=e=>{if(e.target===modal)close();};
    modal.querySelector('#crm1FollowupSave').onclick=async()=>{
      const date=modal.querySelector('#crm1FollowupDate').value;const note=modal.querySelector('#crm1FollowupNote').value.trim();
      if(!date){alert('Please select follow-up date and time.');return;}
      const save=modal.querySelector('#crm1FollowupSave');save.disabled=true;save.textContent='Saving...';
      const iso=new Date(date).toISOString();
      const {error}=await db.from('followups').insert({order_id:orderId,assigned_to:me?.id||null,followup_at:iso,note:note||'Customer follow-up',status:'pending'});
      if(error){alert(error.message);save.disabled=false;save.textContent='Schedule';return;}
      const {error:orderError}=await db.from('orders').update({next_followup_at:iso}).eq('id',orderId);
      if(orderError)console.warn('next_followup_at update failed',orderError);
      close();alert('Follow-up scheduled successfully.');
    };
  }
  ...
  async function renderVerification(){
    ...
    try{
      ...
      c.querySelectorAll('.crm1FollowFromVerify').forEach(b=>b.onclick=()=>followupModal(b.dataset.id));
      c.querySelectorAll('.crm1VerifyFix').forEach(b=>b.onclick=async()=>{
        const v=b.dataset.v;if(v==='failed'&&!confirm('Reject this order verification?'))return;
        b.disabled=true;
        const payload={verification_status:v,verified_by:me?.id||null,verified_at:new Date().toISOString()};
        if(v==='verified')payload.order_status='new';
        const {error:updateError}=await db.from('orders').update(payload).eq('id',b.dataset.id);
